...
export default function PaymentPage() {
  ...
  useEffect(() => {
    ...
    const fetchKYCData = async (emailToFetch) => {
      try {
        ...
        // --- MODIFIED: Simplified Validation Logic ---
        if (isRefillPayment) {
          // If this is a refill, we trust that the user was sent here correctly
          // from the dashboard. No need to check for a temporary status.
          setFormData(prev => ({ ...prev, amountDue: 900, paymentType: 'gas_refill' }));
        } else {
          // For an initial payment, the original check is still required.
    ...
    // The logic to decide whether to fetch or use localStorage data can remain the same.
    // The core change is within `fetchKYCData`.
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      if (isRefillPayment) {
        // This endpoint correctly updates AutoBooking and GasLevel models
        ...
        // This endpoint correctly updates the KYC status to 'active'
