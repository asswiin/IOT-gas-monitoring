...
export default function PaymentPage() {
  ...
  // ✅ NEW: Detect if the user is coming from the manual rebook flow
  const isFromManualRebook = location.state?.fromManualRebook || false;
  ...
  const handleGoBack = async () => {
    // If user came from manual rebook, cancel the booking before going back
    if (isFromManualRebook && userEmail) {
      try {
        // This silently cancels the booking they just created
        await axios.put(getEndpoint.cancelBooking(userEmail));
        console.log("Abandoned manual booking has been cancelled.");
      } catch (error) {
        // Log error but navigate anyway to not trap the user
        console.error("Could not cancel abandoned manual booking:", error);
      }
    }
  ...
  return (
    <div className="payment-container">
      <div className="payment-card">
        ...
        {isRefillPayment && (
          <button 
            onClick={handleGoBack} // ✅ MODIFIED: Use the new handler
