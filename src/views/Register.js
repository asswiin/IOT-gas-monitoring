...
export default function Registration() {
  ...
  const handleChange = (e) => {
    ...
    if (name === "email") {
      const lowerCaseEmail = value.toLowerCase(); // force lowercase
      const emailRegex = /^[^\s@]+@(gmail\.com|yahoo\.com|outlook\.com|hotmail\.com|live\.com|icloud\.com|zoho\.com|protonmail\.com|gmx\.com|yandex\.com)$/;
  ...
  const validateField = (e) => {
    ...
    if (name === "email") {
      ...
      if (!value) {
        ...
        setEmailError("Invalid email provider.");
        e.target.setCustomValidity("Invalid email provider.");
      } else {
        setEmailError("");
        e.target.setCustomValidity("");
      }
