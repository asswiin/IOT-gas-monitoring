...
// function UserDetail({ user, onBack, onDeleteUser }) { // ✅ Added onDeleteUser prop
//   if (!user) return null;
...
//       // ✅ Use a more sophisticated notification system if possible, instead of alert
//       alert("Request approved successfully!"); 
//       fetchPendingRequests();
//       fetchAllUsers(); // Refresh user list after approval
...
//           alert("Request rejected and user data (and payments) removed successfully!");
//           fetchPendingRequests();
//           fetchAllUsers(); // Refresh user list after rejection
//           setSelectedRequest(null);
//           setActiveSection('requests-list');
//       } else {
...
export default function Dashboard() {
  ...
  const handleDeleteUser = async (userEmail) => {
    try {
      await axios.delete(`http://localhost:5000/api/newconnection/${userEmail}`);
      // The backend route `/:email` now handles cascade deletion of payments,
      // so you don't need a separate call to `/api/payment/kyc/${userEmail}` here.
      alert("User and all associated data deleted permanently!");
      fetchAllUsers(); // Refresh user list
      setSelectedUser(null);
      setActiveSection('users');
    } catch (err) {
      console.error("Error deleting user:", err);
      alert("Failed to delete user and associated data."); // Changed to alert for consistency with other messages
    }
  };
