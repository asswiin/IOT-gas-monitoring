      ...
      {!loading && allPayments.length > 0 && (
        <div className="payments-list">
          {allPayments.map((payment) => (
            ...
            >
              {/* ✅ Improved display in the list */}
              <h4>{payment.customerName}</h4>
              <p><strong>Email:</strong> {payment.email}</p>
              <p><strong>Amount:</strong> ₹{payment.amountDue}</p>
              <p><strong>Date:</strong> {new Date(payment.dateOfPayment || payment.createdAt).toLocaleDateString()}</p> {/* Use either payment date or creation date */}
              <p><em>Click to view full details</em></p>
            </div>
          ))}
        </div>
