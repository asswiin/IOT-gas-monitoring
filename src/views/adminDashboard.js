...
export default function Dashboard() {
  ...
  const [complaintsFeedback, setComplaintsFeedback] = useState([]);
  const [otherFeedback, setOtherFeedback] = useState([]);

  // ✅ NEW: State for notification counts
  const [newUrgentFeedbackCount, setNewUrgentFeedbackCount] = useState(0);
  ...
  const fetchData = async () => {
    try {
      const [
        ...
        allBookingsRes, fulfilledBookingsRes, cancelledBookingsRes, feedbackRes
      ] = await Promise.all([
      ...
      // ✅ MODIFIED: Logic for "new refill" notification
      ...
      const allFeedbackData = feedbackRes.data;
      setAllFeedback(allFeedbackData);
      const urgentFeedbacks = allFeedbackData.filter(fb => fb.type === 'Urgent');
      setUrgentFeedback(urgentFeedbacks);
      setComplaintsFeedback(allFeedbackData.filter(fb => fb.type === 'Complaint'));
      setOtherFeedback(allFeedbackData.filter(fb => fb.type !== 'Urgent' && fb.type !== 'Complaint'));

      // ✅ MODIFIED: Logic for "new urgent" notification
      const lastSeenUrgentCount = parseInt(localStorage.getItem('lastSeenUrgentCount') || '0');
      const currentUrgentCount = urgentFeedbacks.length;
      if (currentUrgentCount > lastSeenUrgentCount) {
        setNewUrgentFeedbackCount(currentUrgentCount - lastSeenUrgentCount);
      }
    ...
    switch (activeSection) {
      ...
      case 'my-feedback':
        const renderFeedbackList = (feedbackItems) => {
            const filteredItems = filterBySearch(feedbackItems, searchQueries.feedback, ['email', 'message']);
            return (
                <>
                    <ResultsCount total={feedbackItems.length} filtered={filteredItems.length} query={searchQueries.feedback} />
                    <div className="list-container">
                        ...
                        )) : <p className="no-results">No feedback messages found in this category.</p>}
                    </div>
                </>
            );
        };
        ...
        return (
          <div>
            <div className="section-tabs">
              <button className={`tab-btn ${activeSubSection === 'urgent' ? 'active' : ''}`} onClick={() => setActiveSubSection('urgent')}>
                Urgent ({urgentFeedback.length})
              </button>
              <button className={`tab-btn ${activeSubSection === 'complaints' ? 'active' : ''}`} onClick={() => setActiveSubSection('complaints')}>
                Complaints ({complaintsFeedback.length})
              </button>
              <button className={`tab-btn ${activeSubSection === 'other' ? 'active' : ''}`} onClick={() => setActiveSubSection('other')}>
                General Feedback ({otherFeedback.length})
              </button>
            </div>
            
            <SearchBar section="feedback" placeholder="Search feedback by email or content..." value={searchQueries.feedback} onChange={handleSearchChange} onClear={handleSearchChange} />

            {activeSubSection === 'urgent' && renderFeedbackList(urgentFeedback)}
            {activeSubSection === 'complaints' && renderFeedbackList(complaintsFeedback)}
            {activeSubSection === 'other' && renderFeedbackList(otherFeedback)}
          </div>
        );
  ...
  const getSectionTitle = () => {
    ...
    const titles = {
      ...
      'my-feedback': "Feedback Management"
    };
      ...
      <aside className="sidebar">
        ...
        <nav>
          <ul>
            ...
            {/* ✅ MODIFIED: Click handler clears the notification count */}
            <li className={activeSection === 'my-feedback' ? 'active' : ''} onClick={() => { 
              handleSidebarNav('my-feedback'); 
              setActiveSubSection('urgent');
              setNewUrgentFeedbackCount(0); // Clear notification immediately
              localStorage.setItem('lastSeenUrgentCount', urgentFeedback.length.toString()); // Mark all as seen
            }}>
              Feedback {newUrgentFeedbackCount > 0 && <span className="pending-count urgent-count">({newUrgentFeedbackCount})</span>}
            </li>
          </ul>
        </nav>
